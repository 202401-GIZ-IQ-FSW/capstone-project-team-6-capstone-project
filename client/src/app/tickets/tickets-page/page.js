        ...
        <div className="flex justify-center mx-auto px-4 py-4">
          <div className="flex flex-col items-center gap-1">
            ...
            <div className="w-screen lg:w-full py-4 px-6">
              <div className="flex items-center justify-between p-4 rounded-lg border-gray-500 border-2 gap-4">
            ...
            { tickets.length > 0 ? (
              <div className="w-screen px-6 xl:w-full lg:px-6">
                    ...
                    <table className="divide-y divide-gray-200">
                      ...
                      <thead>
                        <tr className="text-xs lg:text-sm bg-gray-400 text-gray-800 align-top text-left font-medium uppercase tracking-wider">
                          ...
                          {user?.role !== "customer" && <th className="px-3 py-3 text-left ">Role</th>}
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {tickets?.map((ticket, index) => (
                          ...
                          <tr key={index} onClick={() => handleRowClick(ticket?._id)} className="text-xs lg:text-sm hover:bg-gray-300 cursor-pointer">
                            ...
                            {user?.role !== "customer" && <td className="px-3 py-3">{userRoleDisplay(ticket?.user?.role)}</td>}
                            ...
                            { ticket?.assignedUser && ( ticket?.assignedUser?._id === user?._id || user?.role === "superAdmin" || ( user?.role === "admin" && !["admin", "superAdmin"].includes(ticket?.assignedUser?.role) ) ) ?
                              ...
                              :
                              (<td className="px-3 py-3">{ ticket.assignedUser ? ticket.assignedUser?.name : "None" }</td>)
                            }

                            <td className="px-3 py-3">{ ticket.assignedUser ? userRoleDisplay(ticket.assignedUser?.role) : "None" }</td>
